...
import {
  ...
  regimeFor, blocksTouched, rowAreas, areaHistogram, statsFromHistogram, addedSince,
...
async function updateScope() {
  ...
  try {
    ...
    // smaller than one block still costs a whole one (see count.js).
    const tile = {width: image.getTileWidth(), height: image.getTileHeight()};
    if (regimeFor(def.kind, blocksTouched(win, tile)) !== 'near') {
      clearTimeout(slow);
      settle(far());
      return;
    }
