...
import {
  ...
  regimeFor, rowAreas, areaHistogram, statsFromHistogram, addedSince,
...
async function updateScope() {
  ...
  try {
    ...
    if (!win) { settle({regime: 'near', label: describe('near', names, total), stats: zero()}); return; }
    if (regimeFor(def.kind, win.pixels) !== 'near') { settle(far()); return; }
    ...
    const [values] = await image.readRasters({
      window: [win.left, win.top, win.right, win.bottom], samples: [0],
    });
    ...
    // A failed read must not blank the number or leave the previous view's one in
    // place. Falling back to the island sums is honest because the caption then
    // names the islands rather than claiming to describe the screen.
    console.error(err);
    settle(far());
  }
}
